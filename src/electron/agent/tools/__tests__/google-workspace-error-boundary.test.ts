...
beforeEach(() => {
  vi.clearAllMocks();
  settingsSpy.mockReturnValue({ enabled: true, accessToken: 'token', refreshToken: 'refresh', clientId: 'client' });
});
...
describe('GmailTools error boundary', () => {
  it('maps 401 errors to reconnect guidance', async () => {
    ...
    gmailRequestMock.mockRejectedValueOnce(Object.assign(new Error('Unauthorized'), { status: 401 }));
  ...
  it('maps token refresh errors to auth guidance', async () => {
    ...
    const tools = new GmailTools(workspace, daemon as any, taskId);
    const gmailRequestMock = gmailRequest as unknown as ReturnType<typeof vi.fn>;
    gmailRequestMock.mockRejectedValueOnce(new Error('Google Workspace token refresh failed: invalid_grant'));
  ...
  it('logs and rethrows non-auth errors', async () => {
    ...
    await expect(tools.executeAction({ action: 'get_profile' })).rejects.toThrow('Gmail API error 500: nope');
...
describe('GoogleCalendarTools error boundary', () => {
  it('maps 401 errors to reconnect guidance', async () => {
    ...
    calendarRequestMock.mockRejectedValueOnce(Object.assign(new Error('Unauthorized'), { status: 401 }));
  ...
  it('logs and rethrows non-auth errors', async () => {
    ...
    await expect(tools.executeAction({ action: 'list_calendars' })).rejects.toThrow('Google Calendar API error 500: nope');
