...
export class XTools {
  ...
  private static readonly WRITE_ACTIONS = new Set<XAction>(['tweet', 'reply', 'follow', 'unfollow']);
  ...
  private isLikelyBlockingError(message: string): boolean {
    ...
    if (isGenericBlockedOnly && !/rate|suspend|lock|unauthori|forbid|captcha|challenge|verify|access/i.test(message)) {
      return false;
    }
  ...
  private async extractBrowserReadableContent(action: XAction): Promise<BrowserFallbackTweetItem[]> {
    const script = `
      (async () => {
  ...
  private async runBrowserScript(script: string): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private async tryComposeInBrowser(text: string, mode: 'tweet' | 'reply'): Promise<{
    success: boolean;
    draft: boolean;
    submitted: boolean;
    details?: {
    ...
    const buttonLabels = mode === 'reply'
      ? XTools.REPLY_BUTTON_LABELS
      : XTools.POST_BUTTON_LABELS;
  ...
  private async tryToggleFollowButton(action: 'follow' | 'unfollow', userHandle?: string): Promise<{
    success: boolean;
    details?: {
      selector?: string;
      changed?: boolean;
  ...
  private async runBrowserWriteFallback(action: XWriteAction, input: XActionInput, reason: string): Promise<any> {
    ...
    try {
      ...
      const contentText = typeof writePageContent?.text === 'string' ? writePageContent.text : undefined;
    ...
    if (writePageLikelyBlocked) {
      return {
        ...
        fallback: {
          ...
          method: 'browser',
          manualAction: 'Resolve any challenge/login in the opened browser, then retry this X action.',
    ...
    if (!isWriteReady) {
      return {
        ...
        fallback: {
          ...
          navigationStatus: navResult?.isError ? 'partial' : 'loaded',
          method: 'browser',
          manualAction: 'Keep the browser open and retry once the compose/follow controls are visible.',
    ...
    if (action === 'tweet' || action === 'reply') {
      if ((input.media || []).length > 0) {
        return {
          success: false,
          action,
          output: this.buildFallbackMessage(action, reason) + ' Media upload is not supported in automated browser fallback yet.',
          fallback: {
            ...this.buildFallbackDetails(action, input),
            requestedUrl: fallbackUrl,
            navigatedUrl: navResult?.url,
  ...
  private async runBrowserFallback(action: XAction, input: XActionInput, reason: string): Promise<any> {
    if (this.isWriteAction(action)) {
    ...
    if (!isReady) {
      return {
        ...
        fallback: {
          ...
          navigationStatus: navResult?.isError ? 'partial' : 'loaded',
          browserChannel: openResult.browserChannel,
          manualAction: 'Browser content appears slow to load. Keep page open, wait, then retry if needed.',
        },
      };
    }
    ...
    const text = typeof content?.text === 'string' ? this.trimTextForPrompt(content.text) : undefined;
    ...
    const extractionWarning = !hasExtractedItems && !this.isWriteAction(action)
    ...
    return {
      ...
      fallback: {
        ...
        navigationStatus: navResult?.isError ? 'partial' : 'loaded',
        browserChannel: openResult.browserChannel,
        manualAction: likelyBlocked ? 'Keep the browser open and complete any login/challenge flow, then retry.' : undefined,
      },
  ...
  private normalizeHandle(handle?: string): string | undefined {
    ...
    if (likelyUrl.test(candidateWithScheme)) {
      try {
        ...
        const blockedSegments = ['home', 'explore', 'search', 'notifications', 'i', 'status', 'intent'];
        if (!firstSegment || blockedSegments.includes(firstSegment.toLowerCase())) {
          return undefined;
        }

        const normalizedHandle = firstSegment.startsWith('@') ? firstSegment.slice(1) : firstSegment;
        if (!/^[a-zA-Z0-9_]{1,15}$/i.test(normalizedHandle)) {
          return undefined;
        }
    ...
    const blockedHandles = ['home', 'explore', 'search', 'notifications', 'i', 'status', 'intent', 'messages', 'compose', 'settings', 'account', 'help', 'login', 'signup'];
  ...
  async executeAction(input: XActionInput): Promise<any> {
    ...
    for (let attempt = 0; attempt < XTools.COMMAND_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (!this.shouldRetryCommand(action) || !this.isRetryableCommandError(errorMessage) || attempt + 1 >= XTools.COMMAND_RETRY_ATTEMPTS) {
          throw error;
        }
