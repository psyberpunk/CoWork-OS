...
class CheckpointStore {
  ...
  async saveCheckpoint(sessionDir: string, sessionId: string, label?: string): Promise<CanvasCheckpoint> {
  ...
  async restoreCheckpoint(sessionDir: string, sessionId: string, checkpointId: string): Promise<CanvasCheckpoint> {
...
describe('Canvas Checkpoint System', () => {
  ...
  describe('saveCheckpoint', () => {
    ...
    it('throws if session directory does not exist', async () => {
      await expect(
        store.saveCheckpoint('/nonexistent/dir', sessionId)
  ...
  describe('restoreCheckpoint', () => {
    ...
    it('throws if no checkpoints exist for the session', async () => {
      await expect(
        store.restoreCheckpoint(tmpDir, sessionId, 'bogus-id')
    ...
    it('throws if checkpoint ID is not found', async () => {
      ...
      await expect(
        store.restoreCheckpoint(tmpDir, sessionId, 'nonexistent')
