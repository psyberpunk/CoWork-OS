...
const makeRole = (overrides: Partial<{
  id: string;
  name: string;
  displayName: string;
  description: string;
...
describe('ACPAgentRegistry', () => {
  ...
  describe('local agents', () => {
    it('converts active roles to agent cards', () => {
      const roles = [makeRole(), makeRole({ name: 'reviewer', displayName: 'Reviewer', capabilities: ['review'] })];
  ...
  describe('discover', () => {
    ...
    it('filters by query', () => {
      registry.registerRemoteAgent({ name: 'Data Analyst', description: 'Analyzes data', skills: ['pandas'] });
      const roles = [makeRole()];
