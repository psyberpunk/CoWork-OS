...
describe('ACP Handler', () => {
  ...
  describe('acp.discover', () => {
    ...
    it('rejects unauthenticated clients', async () => {
      await expect(
        server.invoke(ACPMethods.DISCOVER, {}, unauthClient)
  ...
  describe('acp.agent.unregister', () => {
    it('unregisters a remote agent', async () => {
      ...
      const result = await server.invoke(ACPMethods.AGENT_UNREGISTER, { agentId: reg.agent.id }) as any;
      expect(result.ok).toBe(true);
      expect(server.broadcast).toHaveBeenCalledWith('acp.agent.unregistered', { agentId: reg.agent.id });
    });
